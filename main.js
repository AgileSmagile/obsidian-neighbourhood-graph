...
__export(main_exports, {
  default: () => NeighbourhoodGraphPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian3 = require("obsidian");

// src/neighbourhood-view.ts
var import_obsidian = require("obsidian");

// src/graph-data.ts
var RELATION_STRENGTH_BONUS = {
  parent: 3,
  child: 3,
  leftFriend: 1,
  rightFriend: 1,
...
function buildNeighbourhood(focusFile, app, settings, excalibrainFields = null) {
  ...
  for (const [sourcePath, targets] of Object.entries(allResolved)) {
    for (const targetPath of Object.keys(targets)) {
      if (!reverseLinks.has(targetPath))
        reverseLinks.set(targetPath, /* @__PURE__ */ new Set());
  ...
  const focusInLinks = (_a = reverseLinks.get(focusFile.path)) != null ? _a : /* @__PURE__ */ new Set();
  const focusAllLinks = /* @__PURE__ */ new Set([...focusOutLinks, ...focusInLinks]);
  ...
  nodeMap.set(focusFile.path, {
    ...
    path: (_c = (_b = focusFile.parent) == null ? void 0 : _b.path) != null ? _c : "",
    focus: true,
    strength: 0
  });
  ...
  for (const file of allFiles) {
    ...
    for (const tag of tags) {
      if (!tagToNotes.has(tag))
        tagToNotes.set(tag, /* @__PURE__ */ new Set());
      tagToNotes.get(tag).add(file.path);
    }
  }
  const neighbourStrength = /* @__PURE__ */ new Map();
  const focusSharedTags = /* @__PURE__ */ new Set();
    ...
    if (notesWithTag && notesWithTag.size > 1) {
      ...
      for (const notePath of notesWithTag) {
        if (notePath !== focusFile.path) {
          neighbourStrength.set(notePath, ((_d = neighbourStrength.get(notePath)) != null ? _d : 0) + 1);
        }
      }
    }
  }
  for (const linkedPath of focusOutLinks) {
    neighbourStrength.set(linkedPath, ((_e = neighbourStrength.get(linkedPath)) != null ? _e : 0) + 2);
  }
  for (const linkedPath of focusInLinks) {
    neighbourStrength.set(linkedPath, ((_f = neighbourStrength.get(linkedPath)) != null ? _f : 0) + 2);
  }
  for (const linkedPath of focusOutLinks) {
    if (focusInLinks.has(linkedPath)) {
      neighbourStrength.set(linkedPath, ((_g = neighbourStrength.get(linkedPath)) != null ? _g : 0) + 2);
    }
  }
  for (const [notePath] of neighbourStrength) {
    const outCount = Object.keys((_h = allResolved[notePath]) != null ? _h : {}).length;
    const inCount = ((_i = reverseLinks.get(notePath)) != null ? _i : /* @__PURE__ */ new Set()).size;
    const hubScore = Math.log2(Math.max(1, outCount + inCount));
    neighbourStrength.set(notePath, ((_j = neighbourStrength.get(notePath)) != null ? _j : 0) + hubScore);
  }
  if (excalibrainFields) {
    for (const { relType, targetPath } of getTypedFrontmatterLinks(focusFile, app, excalibrainFields)) {
      if (neighbourStrength.has(targetPath)) {
        neighbourStrength.set(targetPath, ((_k = neighbourStrength.get(targetPath)) != null ? _k : 0) + RELATION_STRENGTH_BONUS[relType]);
      }
    }
    for (const [notePath] of neighbourStrength) {
      ...
      for (const { relType, targetPath } of getTypedFrontmatterLinks(file, app, excalibrainFields)) {
        if (targetPath === focusFile.path) {
          neighbourStrength.set(notePath, ((_l = neighbourStrength.get(notePath)) != null ? _l : 0) + RELATION_STRENGTH_BONUS[relType]);
        }
      }
    }
  }
  const sorted = [...neighbourStrength.entries()].sort((a2, b) => b[1] - a2[1]);
  ...
  for (const [notePath, strength] of capped) {
    ...
    nodeMap.set(notePath, {
      ...
      type: "note",
      label: file.basename,
      path: (_n = (_m = file.parent) == null ? void 0 : _m.path) != null ? _n : "",
      strength
    });
  }
  ...
  for (const linkedPath of focusAllLinks) {
    if (!neighbourSet.has(linkedPath))
      continue;
    const relType = excalibrainFields ? detectRelationType(focusFile.path, linkedPath, app, excalibrainFields) : void 0;
    addEdge(focusFile.path, linkedPath, relType);
  }
  return {
    nodes: [...nodeMap.values()],
    edges,
    truncated: truncated > 0 ? truncated : void 0
  };
  function addEdge(source, target, relationType) {
...
function getTypedFrontmatterLinks(file, app, fieldLookup) {
  var _a, _b, _c, _d;
  const cache = app.metadataCache.getFileCache(file);
  if (!cache)
    return [];
  const seen = /* @__PURE__ */ new Map();
  for (const fmLink of (_a = cache.frontmatterLinks) != null ? _a : []) {
    const relType = fieldLookup.get(fmLink.key.toLowerCase());
    if (!relType)
      continue;
    const resolved = (_b = app.metadataCache.getFirstLinkpathDest(fmLink.link, file.path)) == null ? void 0 : _b.path;
    if (resolved)
      seen.set(resolved, relType);
  }
  for (const [key, value] of Object.entries((_c = cache.frontmatter) != null ? _c : {})) {
    const relType = fieldLookup.get(key.toLowerCase());
    if (!relType)
      continue;
    const rawNames = Array.isArray(value) ? value.filter((v) => typeof v === "string") : typeof value === "string" ? [value] : [];
    for (const name of rawNames) {
      ...
      if (!stripped)
        continue;
      const resolved = (_d = app.metadataCache.getFirstLinkpathDest(stripped, file.path)) == null ? void 0 : _d.path;
...
function detectRelationType(sourcePath, targetPath, app, fieldLookup) {
...
var FOCUS_R_BONUS = 0.2;
var TAG_COLOUR = "#d4a017";
var HIGHLIGHT_COLOUR = "#fbbf24";
var RELATION_DASH = {
  parent: null,
  // solid — structural hierarchy
  child: null,
  // solid — structural hierarchy
  leftFriend: "5 3",
  // dashed — lateral association
  rightFriend: "2 3",
  // dotted — opposing/contrast
  previous: "8 3 2 3",
  // dash-dot — sequential
  next: "8 3 2 3"
  // dash-dot — sequential
...
function greyFromSlider(v) {
  const level = Math.round(220 - v * 2.05);
  return `rgb(${level},${level},${level})`;
}
function widthFromSlider(v) {
  return 0.5 + (v - 1) * (2.5 / 9);
}
function chargeFromSlider(v) {
  return -50 - (v - 1) * (450 / 9);
}
function distanceFromSlider(v) {
  return 50 + (v - 1) * (150 / 9);
}
function computeRadii(nodes, settings) {
  var _a;
  ...
  const strengths = noteNodes.map((n) => {
    var _a2;
    return (_a2 = n.strength) != null ? _a2 : 0;
  });
  ...
  for (const node of nodes) {
    if (node.type !== "note") {
      ...
      const s = (_a = node.strength) != null ? _a : 0;
...
function matchesColourGroup(node, group) {
  var _a;
  const q = group.query.trim().toLowerCase();
  if (q.startsWith("path:")) {
    const prefix = q.slice(5);
    return ((_a = node.path) != null ? _a : "").toLowerCase().startsWith(prefix);
  }
  if (q.startsWith("tag:")) {
    const tag = q.slice(4);
...
function getNodeColour(node, settings, defaultColour) {
  if (node.type === "tag")
    return TAG_COLOUR;
  for (const group of settings.colourGroups) {
    if (matchesColourGroup(node, group))
      return group.colour;
  }
  return defaultColour;
}
function getEdgeNodeId(node) {
...
var GraphRenderer = class {
  ...
  render(data) {
    ...
    const defaultNodeColour = this.settings.defaultNodeColour !== "#6b7280" ? this.settings.defaultNodeColour : themeAccent || "#6b7280";
    ...
    this.svg = select_default2(this.container).append("svg").attr("width", W).attr("height", H).classed("neighbourhood-graph-svg", true);
    const defs = this.svg.append("defs");
    const filter2 = defs.append("filter").attr("id", "nh-link-glow").attr("x", "-50%").attr("y", "-50%").attr("width", "200%").attr("height", "200%");
    filter2.append("feGaussianBlur").attr("in", "SourceGraphic").attr("stdDeviation", "2.0").attr("result", "blur");
    const merge = filter2.append("feMerge");
    merge.append("feMergeNode").attr("in", "blur");
    merge.append("feMergeNode").attr("in", "SourceGraphic");
    const focusGlow = defs.append("filter").attr("id", "nh-focus-glow").attr("x", "-100%").attr("y", "-100%").attr("width", "300%").attr("height", "300%");
    focusGlow.append("feGaussianBlur").attr("in", "SourceGraphic").attr("stdDeviation", "4").attr("result", "blur");
    ...
    this.svg.call(zoom);
    const nodes = data.nodes.map((n) => ({ ...n, r: 0 }));
    const edges = data.edges.map((e) => ({ ...e }));
    computeRadii(nodes, this.settings);
    const noteRMax = this.settings.maxNodeSize;
    const noteRMin = Math.max(2, Math.round(noteRMax * 0.2));
    this.simulation = simulation_default(nodes).force("link", link_default(edges).id((d) => d.id).distance(distanceFromSlider(this.settings.linkPull)).strength(0.55)).force("charge", manyBody_default().strength(chargeFromSlider(this.settings.spread))).force("center", center_default(W / 2, H / 2).strength(0.04)).force("collision", collide_default((d) => d.r + 24)).force("radial", radial_default(
      (d) => d.type === "note" ? Math.min(W, H) * 0.34 : 0,
      W / 2,
      H / 2
    ).strength((d) => d.type === "note" ? 0.12 : 0.06));
    const link = g.append("g").selectAll("line").data(edges).join("line").attr("stroke", initLineColour).attr("stroke-width", initLineWidth).attr("stroke-opacity", 0.9).attr("stroke-dasharray", (e) => {
      const dash = e.relationType ? RELATION_DASH[e.relationType] : null;
      return dash != null ? dash : null;
    });
    const linkHitArea = g.append("g").selectAll("line").data(edges.filter((e) => !!e.relationType)).join("line").attr("stroke", "transparent").attr("stroke-width", 14).attr("fill", "none").style("cursor", "default");
    ...
    const highlightNode = (d) => {
      ...
      if (depth === 2) {
        edges.forEach((edge) => {
          const srcId = getEdgeNodeId(edge.source);
          const tgtId = getEdgeNodeId(edge.target);
          if (directIds.has(srcId) && tgtId !== d.id)
            secondaryIds.add(tgtId);
          if (directIds.has(tgtId) && srcId !== d.id)
            secondaryIds.add(srcId);
        });
        directIds.forEach((id2) => secondaryIds.delete(id2));
      }
      const edgeLinkType = (e) => {
        ...
        if (depth === 2) {
          ...
          if (srcDirect && secondaryIds.has(tgtId) || tgtDirect && secondaryIds.has(srcId))
            return "secondary";
        }
        return "none";
      };
      link.attr("stroke", (e) => edgeLinkType(e) !== "none" ? HIGHLIGHT_COLOUR : currentLineColour).attr("stroke-width", (e) => {
        const t = edgeLinkType(e);
        return t === "primary" ? Math.max(currentLineWidth + 1, 2) : t === "secondary" ? Math.max(currentLineWidth + 0.5, 1) : currentLineWidth;
      }).attr("stroke-opacity", (e) => {
        const t = edgeLinkType(e);
        return t === "primary" ? 0.8 : t === "secondary" ? 0.35 : 0.2;
      }).attr("filter", (e) => edgeLinkType(e) === "primary" ? "url(#nh-link-glow)" : null);
      node.attr("opacity", (n) => {
        if (n.id === d.id || directIds.has(n.id))
          return 1;
        if (depth === 2 && secondaryIds.has(n.id))
          return 0.7;
        return 0.35;
      });
    };
    const clearHighlight = () => {
      link.attr("stroke", currentLineColour).attr("stroke-width", currentLineWidth).attr("stroke-opacity", 0.9).attr("stroke-dasharray", (e) => {
        const dash = e.relationType ? RELATION_DASH[e.relationType] : null;
        return dash != null ? dash : null;
      }).attr("filter", null);
      node.attr("opacity", 1);
    };
    const node = g.append("g").selectAll("g").data(nodes).join("g").attr("cursor", (d) => d.type === "note" && !d.focus ? "pointer" : "default").call(
      drag_default().on("start", (e, d) => {
        if (!e.active && this.simulation)
        ...
        if (!e.active && this.simulation)
          this.simulation.alpha(5e-3).alphaTarget(0);
        d.fx = null;
        d.fy = null;
        clearHighlight();
      })
    ).on("mouseenter", (_, d) => highlightNode(d)).on("mouseleave", () => clearHighlight()).on("click", (_, d) => {
      if (d.type !== "note")
        return;
      if (this._clickTimer) {
    ...
    node.each(function(d) {
      ...
      if (d.type === "note") {
        ...
        if (d.focus) {
          sel.append("circle").attr("r", d.r + 4).attr("fill", "none").attr("stroke", HIGHLIGHT_COLOUR).attr("stroke-width", 2).attr("stroke-opacity", 0.4).attr("filter", "url(#nh-focus-glow)");
        }
        sel.append("circle").attr("r", d.r).attr("fill", fill).attr("stroke", d.focus ? HIGHLIGHT_COLOUR : "#fff").attr("stroke-width", d.focus ? 3 : 1.5);
      } else {
        const r = d.r;
        sel.append("polygon").attr("points", `0,${-r} ${r},0 0,${r} ${-r},0`).attr("fill", getNodeColour(d, settingsRef, defaultNodeColour)).attr("stroke", "#fff").attr("stroke-width", 1);
      }
    });
    node.append("text").attr("text-anchor", "middle").attr("font-size", (d) => {
      if (d.focus)
        return "12px";
      if (d.type !== "note")
        return "9px";
    ...
    }).attr("font-weight", (d) => d.focus ? "700" : "400").attr("fill", (d) => d.type === "note" ? textColour : tagTextColour).attr("pointer-events", "none").each(function(d) {
      const sel = select_default2(this);
      const words = d.label.split(/[\s-]+/);
      const mid = Math.ceil(words.length / 2);
      const line1 = words.slice(0, mid).join(" ");
      const line2 = words.length > 1 ? words.slice(mid).join(" ") : "";
      sel.append("tspan").attr("x", 0).attr("dy", line2 ? -(d.r + 16) : -(d.r + 4)).text(line1);
      if (line2) {
        sel.append("tspan").attr("x", 0).attr("dy", 12).text(line2);
      }
    });
    const tooltip = select_default2(this.container).append("div").classed("neighbourhood-graph-tooltip", true).style("position", "absolute").style("pointer-events", "none").style("opacity", "0");
    const showPath = this.settings.showPathInTooltip;
    const focusRelation = /* @__PURE__ */ new Map();
    ...
    if (focusNode) {
      for (const e of edges) {
        ...
        const srcId = getEdgeNodeId(e.source);
        const tgtId = getEdgeNodeId(e.target);
        if (srcId === focusNode.id)
          focusRelation.set(tgtId, e.relationType);
        if (tgtId === focusNode.id)
          focusRelation.set(srcId, e.relationType);
      }
    }
    node.on("mouseover", (_, d) => {
      ...
      if (left + tw > rect.width)
        left = e.clientX - rect.left - tw - 12;
      if (top < 0)
        top = e.clientY - rect.top + 24;
      tooltip.style("left", `${left}px`).style("top", `${top}px`);
    }).on("mouseout", () => tooltip.style("opacity", "0"));
    linkHitArea.on("mouseover", (_, e) => {
      if (!e.relationType)
        return;
      const tooltipEl = tooltip.node();
      tooltipEl.empty();
      ...
      if (left + tw > rect.width)
        left = ev.clientX - rect.left - tw - 12;
      if (top < 0)
        top = ev.clientY - rect.top + 24;
      tooltip.style("left", `${left}px`).style("top", `${top}px`);
    }).on("mouseout", () => tooltip.style("opacity", "0"));
    this.simulation.on("tick", () => {
      link.attr("x1", (d) => {
        var _a;
        return (_a = d.source.x) != null ? _a : 0;
      }).attr("y1", (d) => {
        var _a;
        return (_a = d.source.y) != null ? _a : 0;
      }).attr("x2", (d) => {
        var _a;
        return (_a = d.target.x) != null ? _a : 0;
      }).attr("y2", (d) => {
        var _a;
        return (_a = d.target.y) != null ? _a : 0;
      });
      linkHitArea.attr("x1", (d) => {
        var _a;
        return (_a = d.source.x) != null ? _a : 0;
      }).attr("y1", (d) => {
        var _a;
    ...
    this._setLineColour = (v) => {
      currentLineColour = v;
    };
    this._setLineWidth = (v) => {
      currentLineWidth = v;
    };
  }
  updateSlider(name, value) {
    if (!this._link || !this._simulation)
      return;
    switch (name) {
      case "lineColour": {
      ...
      case "spread": {
        this._simulation.force("charge").strength(chargeFromSlider(value));
        this._simulation.alphaTarget(0.3).restart();
        setTimeout(() => {
          var _a;
          return (_a = this._simulation) == null ? void 0 : _a.alphaTarget(0);
        }, 1500);
        break;
      }
      case "linkPull": {
        this._simulation.force("link").distance(distanceFromSlider(value));
        this._simulation.alphaTarget(0.3).restart();
        setTimeout(() => {
          var _a;
          return (_a = this._simulation) == null ? void 0 : _a.alphaTarget(0);
...
async function getExcalibrainState(app) {
  ...
  if (!await app.vault.adapter.exists(mainJs))
    return "not-installed";
  if (!await app.vault.adapter.exists(dataJson))
    return "installed-unconfigured";
  return "configured";
}
async function loadExcalibrainConfig(app) {
  const mainJs = `${app.vault.configDir}/plugins/excalibrain/main.js`;
  const dataJson = `${app.vault.configDir}/plugins/excalibrain/data.json`;
  if (!await app.vault.adapter.exists(mainJs))
    return null;
...
function buildFieldLookup(config) {
  ...
  for (const [fields, type2] of entries) {
    for (const field of fields) {
      map.set(field.toLowerCase(), type2);
    }
  }
  return map;
}
...
// src/neighbourhood-view.ts
var VIEW_TYPE = "neighbourhood-graph";
var NeighbourhoodGraphView = class extends import_obsidian.ItemView {
  constructor(leaf, plugin) {
    ...
    this._relevantPaths = /* @__PURE__ */ new Set();
    this.plugin = plugin;
  }
  getViewType() {
    return VIEW_TYPE;
  ...
  async onOpen() {
    ...
    if (this.excalibrainFields) {
      ...
      for (const hint of edgeHints) {
        ...
        if (hint.dash)
          line.setAttribute("stroke-dasharray", hint.dash);
    ...
    legendHeader.addEventListener("mousedown", (e) => {
      if (e.target.closest(".ng-legend-chevron"))
        return;
      isDragging = true;
    ...
    const onMouseMove = (e) => {
      if (!isDragging || !this.graphContainer)
        return;
      const parentRect = this.graphContainer.getBoundingClientRect();
      let x3 = e.clientX - dragOffsetX;
      let y3 = e.clientY - dragOffsetY;
      const legendRect = legend.getBoundingClientRect();
      x3 = Math.max(0, Math.min(x3, parentRect.width - legendRect.width));
      y3 = Math.max(0, Math.min(y3, parentRect.height - legendRect.height));
      legend.style.left = `${x3}px`;
      legend.style.top = `${y3}px`;
    ...
    this.registerEvent(
      this.app.workspace.on("active-leaf-change", (0, import_obsidian.debounce)(() => {
        this.onActiveLeafChange();
      }, 200, true))
    );
    this.registerEvent(
      this.app.metadataCache.on("changed", (0, import_obsidian.debounce)((file) => {
  ...
  onActiveLeafChange() {
    var _a, _b;
    const activeFile = (_a = this.app.workspace.getActiveFile()) != null ? _a : this.findActiveMarkdownFile();
    if (!activeFile || activeFile.extension !== "md")
      return;
    if (activeFile.path === ((_b = this.focusFile) == null ? void 0 : _b.path))
      return;
    this.focusFile = activeFile;
    this.rebuild();
  }
  findActiveMarkdownFile() {
  ...
  async reloadAndRebuild() {
    await this.refreshExcalibrainFields();
    this.buildSettingsPanel();
    this.rebuild();
  }
  async refreshExcalibrainFields() {
  ...
  rebuild() {
    if (!this.focusFile || !this.graphContainer)
      return;
  ...
  buildSettingsPanel() {
    ...
    this.addSettingWithInfo(
      ...
      (s) => s.addDropdown(
        (drop) => drop.addOption("1", "1 hop").addOption("2", "2 hops").setValue(String(this.plugin.settings.depth)).onChange(async (val) => {
          this.plugin.settings.depth = Number(val);
    ...
    this.addSettingWithInfo(
      ...
      (s) => s.addText((text) => {
        ...
        return text.setValue(String(this.plugin.settings.maxNeighbours)).onChange(async (val) => {
    ...
    (0, import_obsidian.setIcon)(salienceInfo, "info");
    ...
    (0, import_obsidian.setIcon)(nodeSizeInfo, "info");
    ...
    for (const s of sliders) {
      ...
      input.addEventListener("input", () => {
        ...
        if (this.renderer)
          this.renderer.updateSlider(key, v);
        if (key === "lineColour") {
          const c2 = greyFromSlider(v);
          this._legendEdgeLines.forEach((l) => l.setAttribute("stroke", c2));
  ...
  addSettingWithInfo(container, name, tooltip, configure) {
    ...
    (0, import_obsidian.setIcon)(info, "info");
    configure(setting);
  }
  async importColourGroups() {
    try {
      const configPath = `${this.app.vault.configDir}/graph.json`;
      const exists = await this.app.vault.adapter.exists(configPath);
      if (!exists) {
        new import_obsidian.Notice("No graph.json found. Open Obsidian's graph view and configure colour groups first.");
        return;
      }
      const raw = await this.app.vault.adapter.read(configPath);
      const config = JSON.parse(raw);
      if (!config.colorGroups || config.colorGroups.length === 0) {
        new import_obsidian.Notice("No colour groups found in Obsidian's graph view settings.");
        return;
      }
      this.plugin.settings.colourGroups = config.colorGroups.map((g) => ({
        query: g.query.trim(),
      ...
      this.buildSettingsPanel();
      this.rebuild();
      new import_obsidian.Notice(`Imported ${this.plugin.settings.colourGroups.length} colour groups from graph view.`);
    } catch (e) {
      new import_obsidian.Notice("Failed to read graph view settings.");
    }
  }
};
...
var EXCALIBRAIN_PLUGIN_URL = "https://obsidian.md/plugins?id=excalibrain";
function obsidianRgbToHex(rgb2) {
  return `#${rgb2.toString(16).padStart(6, "0")}`;
}
var NeighbourhoodGraphSettingTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    /** Persists collapse state across re-renders triggered by add/remove/reorder */
    this._groupsCollapsed = null;
    this.plugin = plugin;
  }
  display() {
    this.containerEl.empty();
    this.containerEl.createEl("h3", { text: "Graph behaviour" });
    new import_obsidian2.Setting(this.containerEl).setName("Highlight depth").setDesc("How many tiers of connections light up when you hover a node. 1 hop highlights only direct connections. 2 hops also highlights connections of connections.").addDropdown(
      (drop) => drop.addOption("1", "1 hop").addOption("2", "2 hops").setValue(String(this.plugin.settings.depth)).onChange(async (val) => {
        this.plugin.settings.depth = Number(val);
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(this.containerEl).setName("Max neighbours").setDesc("Maximum number of notes shown around the focus note. The most strongly connected neighbours are shown first.").addText((text) => {
      text.inputEl.type = "number";
      text.inputEl.style.width = "60px";
      return text.setValue(String(this.plugin.settings.maxNeighbours)).onChange(async (val) => {
        const num = parseInt(val, 10);
    ...
    new import_obsidian2.Setting(this.containerEl).setName("Max node size").setDesc("Maximum radius (px) of the largest neighbour node. Reduce for a compact sidebar; increase for a full-panel view. Default 10 is optimised for a narrow sidebar.").addSlider(
      (slider) => slider.setLimits(4, 20, 1).setValue(this.plugin.settings.maxNodeSize).setDynamicTooltip().onChange(async (val) => {
        this.plugin.settings.maxNodeSize = val;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(this.containerEl).setName("Show path in tooltip").setDesc("When hovering a note node, show its vault-relative folder path below the title. Useful for distinguishing notes with similar names.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.showPathInTooltip).onChange(async (val) => {
        this.plugin.settings.showPathInTooltip = val;
        await this.plugin.saveSettings();
      })
    );
    this.containerEl.createEl("h3", { text: "Colour groups" });
    new import_obsidian2.Setting(this.containerEl).setName("Default node colour").setDesc("Notes not matching any group use this colour. Defaults to your theme's accent colour.").addColorPicker(
      (picker) => picker.setValue(this.plugin.settings.defaultNodeColour).onChange(async (val) => {
    ...
    const importIconBtn = new import_obsidian2.ButtonComponent(importRow);
    importIconBtn.setIcon("arrow-down-to-line").setTooltip("Import colour groups from Obsidian's built-in graph view").setClass("ng-icon-btn").onClick(async () => {
    ...
    new import_obsidian2.Setting(groupsBody).addButton(
  ...
  async renderExcalibrainSection(container) {
    ...
    if (state === "not-installed") {
      ...
      (0, import_obsidian2.setIcon)(icon, "info");
      ...
      const icon = statusRow.createSpan({ cls: "ng-excali-status-icon" });
      (0, import_obsidian2.setIcon)(icon, "clock");
      ...
      new import_obsidian2.Setting(container).setName("Use Excalibrain relationships").setDesc("Edges where source or target note has a matching frontmatter field will be styled distinctly and ranked higher.").addToggle(
        (toggle) => toggle.setValue(this.plugin.settings.excalibrainEnabled).onChange(async (val) => {
      ...
      new import_obsidian2.Setting(container).setName("Use Excalibrain relationships").setDesc("Read Excalibrain relationship fields to type edges and boost connection strength.").addToggle(
        (toggle) => toggle.setValue(this.plugin.settings.excalibrainEnabled).onChange(async (val) => {
          this.plugin.settings.excalibrainEnabled = val;
          await this.plugin.saveSettings();
        })
  ...
  renderColourGroup(container, index2) {
    const group = this.plugin.settings.colourGroups[index2];
    const setting = new import_obsidian2.Setting(container).addText(
      (text) => text.setPlaceholder("e.g. path:people/").setValue(group.query).onChange(async (val) => {
        this.plugin.settings.colourGroups[index2].query = val;
        await this.plugin.saveSettings();
      })
    ).addColorPicker(
      (picker) => picker.setValue(group.colour).onChange(async (val) => {
        this.plugin.settings.colourGroups[index2].colour = val;
        await this.plugin.saveSettings();
      })
    ).addExtraButton(
      (btn) => btn.setIcon("trash").setTooltip("Remove group").onClick(async () => {
        this.plugin.settings.colourGroups.splice(index2, 1);
        await this.plugin.saveSettings();
        this.display();
      })
    );
    if (index2 > 0) {
      setting.addExtraButton(
        (btn) => btn.setIcon("arrow-up").setTooltip("Move up").onClick(async () => {
          ...
          [groups[index2 - 1], groups[index2]] = [groups[index2], groups[index2 - 1]];
          await this.plugin.saveSettings();
          this.display();
        })
      );
    }
    if (index2 < this.plugin.settings.colourGroups.length - 1) {
      setting.addExtraButton(
        (btn) => btn.setIcon("arrow-down").setTooltip("Move down").onClick(async () => {
          const groups = this.plugin.settings.colourGroups;
          [groups[index2], groups[index2 + 1]] = [groups[index2 + 1], groups[index2]];
          await this.plugin.saveSettings();
          this.display();
        })
      );
  ...
  async importFromGraphView() {
    try {
      const configPath = `${this.app.vault.configDir}/graph.json`;
      const exists = await this.app.vault.adapter.exists(configPath);
      if (!exists) {
        new import_obsidian2.Notice("No graph.json found. Open Obsidian's graph view and configure colour groups first.");
        return;
      }
      const raw = await this.app.vault.adapter.read(configPath);
      const config = JSON.parse(raw);
      if (!config.colorGroups || config.colorGroups.length === 0) {
        new import_obsidian2.Notice("No colour groups found in Obsidian's graph view settings.");
        return;
      }
      const imported = config.colorGroups.map((g) => ({
        query: g.query.trim(),
      ...
      await this.plugin.saveSettings();
      this.display();
      new import_obsidian2.Notice(`Imported ${imported.length} colour group${imported.length === 1 ? "" : "s"} from graph view.`);
    } catch (e) {
      console.warn("[neighbourhood-graph] Failed to import graph colours:", e);
      new import_obsidian2.Notice("Failed to read graph view settings.");
    }
  }
};
...
// src/main.ts
var NG_ICON_ID = "neighbourhood-graph-icon";
var NG_ICON_SVG = `<circle cx="50" cy="50" r="43.8" fill="none" stroke="currentColor" stroke-width="5" stroke-opacity="0.65" stroke-linecap="round"/>
...
var NeighbourhoodGraphPlugin = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    (0, import_obsidian3.addIcon)(NG_ICON_ID, NG_ICON_SVG);
    this.registerView(
      VIEW_TYPE,
